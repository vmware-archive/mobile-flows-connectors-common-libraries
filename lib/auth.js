...
const validator = async (mfPublicKeyUrl, req, res, next) => {
  try {
    ...
    res.locals.mfJwt.preHire = decoded.pre_hire
  } catch (error) {
...
const getPublicKey = async (mfPublicKeyUrl) => {
  ...
  const response = await fetch(mfPublicKeyUrl)
  if (!response.ok) {
    const error = new Error('Failed to fetch Mobile Flows public key ' + response.statusText)
