/*
* Copyright © 2020 VMware, Inc. All Rights Reserved.
* SPDX-License-Identifier: BSD-2-Clause
*/

'use strict'

const jwt = require('jsonwebtoken')

const privateKey = '-----BEGIN RSA PRIVATE KEY-----\\n-----END RSA PRIVATE KEY-----\n'
const publicKey = '-----BEGIN PUBLIC KEY-----\\noQIDAQAB\n-----END PUBLIC KEY-----\n'

let fakeMf

/**
 * A backdoor to obtain Mobile Flows authorization JWT.
 * @param  tokenProperties - Object containing token properties.
 */
const getMfToken = (tokenProperties) => {
  return jwt.sign(
    {
      prn: `${tokenProperties.username}@tenantId`,
      eml: `${tokenProperties.username}@vmware.com`,
      tenant: 'tenantId',
      domain: 'vmware.com',
      pre_hire: tokenProperties.pre_hire
    },
    privateKey,
    {
      algorithm: 'RS256',
      expiresIn: '300 seconds',
      subject: 'todo-user-id',
      audience: tokenProperties.audience,
      issuer: 'todo-mf-server'
    }
  )
}

/**
 * Start the mock Mobile Flows server for testing purposes.
 * The public key will be accessible at "/security/public-key".
 * @param  port - It will listen on this port.
 */
const start = (port) => {
  const express = require('express')
  const app = express()
  app.get('/security/public-key', (req, res) => res.send(publicKey))
  fakeMf = app.listen(port, () => console.log('Fake mobile flows server listening on %s', port))
}

const stop = (fn) => fakeMf.close(fn)

module.exports = {
  start,
  stop,
  getMfToken
}
